...
$SUDO $DOCKER build -t mechatoni/confluence confluence
CONFLUENCE_VERSION="$($SUDO $DOCKER run --rm mechatoni/confluence sh -c 'echo $CONFLUENCE_VERSION')"
$SUDO $DOCKER tag mechatoni/confluence mechatoni/confluence:$CONFLUENCE_VERSION

$SUDO $DOCKER run -d --name confluence --link postgres:db --link jira:jira -p $CONFLUENCE_EXTERNAL_PORT:8090 mechatoni/confluence

echo "Containers running..."
$SUDO $DOCKER ps

echo "IP Addresses of containers:"
$SUDO $DOCKER inspect -f '{{ .Config.Hostname }} {{ .Config.Image }} {{ .NetworkSettings.IPAddress }}' postgres stash jira confluence
